...
struct msg_receiver {
	struct list_head r_list;		// 双向链表实现，一般嵌入
	struct task_struct* r_tsk;		//进程结构体
...
struct msg_msgseg {					//用于嵌入msg_msg结构体，实现链式结构
	struct msg_msgseg* next;
	/* the next part of the message follows immediately */
};
/* one msg_msg structure for each message */
struct msg_msg {		//消息，通过msg_msg结构体
...
static atomic_t msg_bytes = ATOMIC_INIT(0);  //原子变量
...
void __init msg_init (void)		//初始化消息队列
{
...
static int newque (key_t key, int msgflg) //新建消息队列
{
	int id;
	struct msg_queue *msq;

	msq  = (struct msg_queue *) kmalloc (sizeof (*msq), GFP_KERNEL);//分配空间
	if (!msq) 
		return -ENOMEM;
...
static void free_msg(struct msg_msg* msg)  //释放消息队列
{
